...
| `js/music.js` | background music from `assets/music/` via plain `<audio>` (works on `file://`): one looping walking tune (an Oktoberfest folk-song medley, rebuilt by `assets/music/source/make_medley.py` — NOT needed to run the game) + a battle playlist that advances one clip per fight; M mutes |
